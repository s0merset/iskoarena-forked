...
interface SignupPageProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (fullName: string, username: string, password: string) => { success: boolean; message: string };
    onToggleLogin: () => void;
}
...
export default function SignupPage({ isOpen, onClose, onSubmit, onToggleLogin }: SignupPageProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Combine names for your AuthManager
        const fullName = `${firstName} ${lastName}`.trim();
        
        const result = onSubmit(fullName, username, password);
            ...
            <section className="relative w-full max-w-sm animate-in fade-in zoom-in duration-200">
                ...
                >
                    <div className="p-8 pb-6">
                        <div>
                            <LogoIcon />
                            <h1 className="mb-1 mt-4 text-xl font-semibold text-white">Create an Account</h1>
                            <p className="text-sm text-zinc-400">Join IskoArena to get started</p>
                        </div>
                        ...
                        {/* Status Message */}
                        {status && (
                            <div className={`mb-4 p-2 text-xs border rounded ${
                                status.success 
                        ...
                        <div className="space-y-5">
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-2">
                                    <Label htmlFor="firstname" className="block text-sm text-zinc-300">Firstname</Label>
                                    <Input
                                        type="text"
                                        required
                                        id="firstname"
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="lastname" className="block text-sm text-zinc-300">Lastname</Label>
                                    <Input
                                        type="text"
                                        required
                                        id="lastname"
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="username" className="block text-sm text-zinc-300">Username</Label>
                                <Input
                                    type="text"
                                    required
                                    id="username"
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="pwd" className="text-sm text-zinc-300">Password</Label>
                                <Input
                                    type="password"
                                    required
                                    id="pwd"
